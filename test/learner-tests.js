...
var Learner = require('../lib/learner')(inherits, EventEmitter)
...
describe('Learner', function () {
	...
	describe('decide' , function () {
		...
		it('emits a fact when a majority is reached', function (done) {
			...
			learner.decide(proposal0)
			learner.decide(proposal1)
		})
		...
		it('the chosen proposal is stored as a fact', function () {
			var proposal0 = new Proposal(0, 1, 2)
			var proposal1 = new Proposal(1, 1, 3)
			learner.decide(proposal0)
			learner.decide(proposal1)
			assert.equal(learner.instanceFacts[1], proposal1)
		})

		it('skips proposals for known facts', function () {
			var proposal0 = new Proposal(0, 1, 2)
			var proposal1 = new Proposal(1, 1, 3)
			learner.decide(proposal0)
			learner.decide(proposal1)
