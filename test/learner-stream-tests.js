...
function makeFacts(count, start) {
	...
	for (var i = start; i < start + count; i++) {
		facts.push(new Proposal(0, i, 999))
	}
	return facts
}
...
describe('LearnerStream', function () {
	...
	describe('on(data)' , function () {
		...
		afterEach(function () {
			learner.removeAllListeners('fact')
			ls.removeAllListeners('data')
		})

		it('emits data events immediately when learner emits sequentially',
			function (done) {
				var count = 10
				var i = 1
				...
				while (facts.length) {
					learner.emit('fact', facts.shift())
				}
			}
		)

		it('emits data in instance order when learner emits out of order',
			function (done) {
				var count = 100
				var i = 1
		...
		it('emits data in instance order when learner emits in order and out',
			function (done) {
				var count = 100
				var i = 1
